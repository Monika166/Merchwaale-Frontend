...
const Shop: React.FC = () => {
  ...
  const filteredProducts = PRODUCTS.filter((p) => {
    ...
    const matchesSub =
      activeSubCatId === "all" || p.subCategory === activeSubCatId;
    const matchesDept =
      selectedDept.length === 0 ||
      (p.department && selectedDept.includes(p.department.id));
    const matchesKeyword =
      searchKeyword === "" ||
      p.name.toLowerCase().includes(searchKeyword?.toLowerCase());
    return matchesCat && matchesSub && matchesDept && matchesKeyword;
  });
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-wider transition-all border shrink-0 ${
                    activeSubCatId === sub.id
                      ...
                      : "bg-gray-50 dark:bg-gray-800 border-gray-100 dark:border-gray-700 text-text-sub hover:border-primary/50"
                  }`}
                >
                  {sub.name}
                </button>
              ...
              <button
                ...
                className={`text-left px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${
                  activeCatId === "all"
                    ...
                    : "hover:bg-gray-50 dark:hover:bg-gray-800 text-text-main dark:text-gray-300"
                }`}
              >
                Full Catalog
              </button>
                    ...
                    <div className="pl-4 py-1 flex flex-col gap-1 border-l-2 border-primary/10 ml-4 mt-1 animate-in slide-in-from-left-2 duration-300">
                      {cat.subCategories.map((sub) => (
                        <button
                          key={sub.id}
                          onClick={() => handleSubCatClick(sub.id)}
                          className={`text-left px-4 py-1.5 rounded-lg text-[11px] font-bold transition-all ${
                            activeSubCatId === sub.id
                              ? "text-primary bg-primary/5"
                              : "text-text-sub hover:text-text-main dark:hover:text-white hover:bg-gray-50 dark:hover:bg-gray-800"
                          }`}
                        >
                          {sub.name}
                        </button>
                  ...
                  <span
                    className={`text-xs font-bold transition-colors ${
                      selectedDept.includes(dept.id)
                        ? "text-primary"
                        : "text-text-main dark:text-gray-300 group-hover:text-primary"
                    }`}
                  >
                    {dept.name}
                  </span>
          ...
          {filteredProducts.length === 0 ? (
            <div className="py-20 text-center space-y-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-text-sub dark:text-gray-400 mt-1 uppercase tracking-wider">
              ...
              <button
                ...
                className="bg-primary text-white px-6 py-2 rounded-xl text-xs font-black shadow-lg shadow-primary/20 transition-transform"
              >
                Reset All
              </button>
            </div>
